...
class BertForLongSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        if self.rnn_type == "lstm":
            self.rnn = nn.LSTM(input_size=self.hidden_size,hidden_size=self.hidden_size, bidirectional=True if config.finetuning_task != 'spliceai' else False, num_layers=self.num_rnn_layer, batch_first=True, dropout=self.rnn_dropout)
            ...
            raise ValueError
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(self.hidden_size, 3 if config.finetuning_task == 'spliceai' else config.num_labels)
        self.config = config
        self.init_weights()
    ...
    Examples::
        ...
        if self.rnn_type == "lstm":
            ...
            # zero
            if self.config.finetuning_task == 'spliceai':
                out = outputs[0]
                rnn_out = self.rnn(out)[0].reshape(batch_size, self.split * 512, self.hidden_size)
            else:
                pooled_output = outputs[1].view(batch_size, self.split, self.hidden_size)
            ...
            pooled_output = outputs[1].view(batch_size, self.split, self.hidden_size)
            Output, ht = self.rnn(pooled_output)
        else:
            raise ValueError
        ...
        # logits.shape = (bs, seq, 3)
        if labels is not None:
            if self.num_labels == 1:
                loss_fct = MSELoss()
                loss = loss_fct(logits.view(-1), labels.view(-1))
            else:
                p_w = torch.tensor([1.0, 100.0, 100.0]).repeat(logits.shape[0], logits.shape[1], 1)
                loss_fct = BCEWithLogitsLoss(pos_weight=p_w.cuda()) # for deepsea \\ spliceai
